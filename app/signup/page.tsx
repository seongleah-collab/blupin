...
export default function SignupPage() {
  ...
  const handleOAuth = async (provider: Provider) => {
    ...
    if (error || !data?.url) {
      ...
      setError(error?.message || 'could not start sign-up')
      return
    }
    try {
      ...
      if (msg.toLowerCase().includes('not enabled')) {
        setError(`${provider} sign-up isn't fully set up yet — please use email for now.`)
      } else {
        setError(msg || 'sign-up failed — try email instead.')
      }
    } catch {
      window.location.href = data.url
    }
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl md:text-4xl text-white leading-tight mb-2">
            ship before{' '}
            <span className={`${fraunces.className} italic font-medium`}>they do</span>
          </h1>
